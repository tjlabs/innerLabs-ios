...
public class SectorDetectionService: NSObject {
    ...
    var sector = SectorDetectionResult(mobile_time: 0, sector_name: "", calculated_time: 0)
    // ------------------- //
    ...
    func getResult() -> SectorDetectionResult {
        getSector()
        
        return self.sector
    }
    
    internal func getSector() {
        let currentTime: Int = getCurrentTimeInMilliseconds()
        let input = SectorDetection(user_id: uuid, mobile_time: currentTime)
        NetworkManager.shared.postSD(url: SD_URL, input: input, completion: { [self] statusCode, returnedString in
    ...
    func jsonToResult(json: String) -> SectorDetectionResult {
        let result = SectorDetectionResult(mobile_time: 0, sector_name: "", calculated_time: 0)
        let decoder = JSONDecoder()

        let jsonString = json

        if let data = jsonString.data(using: .utf8), let decoded = try? decoder.decode(SectorDetectionResult.self, from: data) {
            return decoded
        }
