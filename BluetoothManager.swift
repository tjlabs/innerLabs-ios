...
public class BluetoothManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    let ServiceUUID   = CBUUID(string: TJLABS_UUID)
    ...
    func startScan(option: BLEScanOption) -> Void {
        ...
        if bluetoothReady {
            if option == .Foreground {
                self.centralManager.scanForPeripherals(withServices: [], options: [CBCentralManagerScanOptionAllowDuplicatesKey : NSNumber(value: true as Bool)])
            }
            else {
                self.centralManager.scanForPeripherals(withServices: [ServiceUUID], options: [CBCentralManagerScanOptionAllowDuplicatesKey : NSNumber(value: true as Bool)])
            }
